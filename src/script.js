...
const geometry1 = new THREE.PlaneGeometry( 10000, 10000 );
                ...
                plane1 = new THREE.Mesh( geometry1, new THREE.MeshBasicMaterial( { color: 0xff0000,visible: true } ) );
...
const currentObject = () => {
    ...
    const result = objects.children.length > 0 ? objects.children[objects.children.length - 1] : null
...
function createObject(intersect) {
    ...
    const basicMaterial = new THREE.MeshBasicMaterial({color: createParams.color})
...
const createGui = ()=>{
    ...
    let changeGeo = create.add(createParams, 'geo', {
        box: 'box',
        cyl: 'cyl',
        pyr: 'pyr'
    })
    ...
    if(currentObject()){
        ...
        modify.add(currentObject().position, 'y').min(-3).max(3).step(0.01).name('elevation').listen()
        modify.add(currentObject().position, 'x').min(-3).max(3).step(0.01).name('currentObject() side to side').listen()
        modify.add(currentObject().position, 'x').min(-3).max(3).step(0.01).name('side to side').listen()
        modify.add(currentObject().position, 'z').min(-3).max(3).step(0.01).name('up and down').listen()
        modify.add(currentObject().rotation, 'y').min(-Math.PI * 2).max(Math.PI * 2).step(Math.PI / 4).name('rotate').listen()
        modify
...
function onPointerDown( event ) {
    ...
    console.log("EXCECUTING POINTER DOWN", event, pointer, raycaster, intersects, allObjects)
    ...
    //} else {
        ...
        console.log("JUST CREATED AN OBJECT AFTER POINTER DOWN", event, isCreationMode)
